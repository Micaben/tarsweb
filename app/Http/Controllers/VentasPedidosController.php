...
class VentasPedidosController extends Controller
{
    ...
    public function obtenerVentasPorMesDolares($mes)
    {       
        $primerDiaMes = Date::createFromFormat('Y-m', $mes)->startOfMonth();
        $ultimoDiaMes = Date::createFromFormat('Y-m', $mes)->endOfMonth();

        $totalVentasDolares = DB::table('cotizacioncab')
            ->where('moneda', '02')
            ->whereBetween('fechacot', [$primerDiaMes, $ultimoDiaMes]) // Asegúrate de que el campo de fecha sea 'fechacot'
            ->sum('total');
        
        return response()->json([
            'totalVentasMoneda02' => (float) $totalVentasDolares,
        ]);
    }

    public function obtenerVentasPorMesSoles($mes)
    {
        // Obtén el primer y último día del mes actual usando Carbon
        $primerDiaMes = Date::createFromFormat('Y-m', $mes)->startOfMonth();
        $ultimoDiaMes = Date::createFromFormat('Y-m', $mes)->endOfMonth();

        // Consulta para sumar el total donde la moneda sea '02' (por ejemplo, dólares)
        $totalVentasSoles = DB::table('cotizacioncab')
            ->where('moneda', '01')
            ->whereBetween('fechacot', [$primerDiaMes, $ultimoDiaMes]) // Asegúrate de que el campo de fecha sea 'fechacot'
            ->sum('total');

        // Retornar el total de ventas en dólares como JSON
        return response()->json([
            'totalVentasMoneda01' => (float) $totalVentasSoles,
        ]);
    }
